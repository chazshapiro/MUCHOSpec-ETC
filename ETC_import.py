...
from numpy import arange, array, hstack, pi, vstack
...
for k in channels:
    ...
    dispersion_scale_nobin[k] = u.pixel_scale(
        (lambdamax - lambdamin) / (Npix_dispers[k] * u.pix)
    )
...
telescope_Area = (
    (1.0 - Obscuration**2) * pi * (telescope_D / 2.0) ** 2
)  # Collecting area
...
def makeBinCenters(binspec, chanlist=channels, wrange=None):
    ...
    for k in chanlist:
        lambdamin, lambdamax = channelRange[k]
        binCenters[k] = rangeQ(
            lambdamin, lambdamax, binspec * (lambdamax - lambdamin) / Npix_dispers[k]
        )
...
def rangeQ(q0, q1, dq=None):
    ...
    assert isinstance(q0, u.Quantity) and isinstance(q1, u.Quantity), (
        "Inputs must be Quantities"
    )
    ...
    assert (unit.physical_type == q1.unit.physical_type) and (
        unit.physical_type == dq.unit.physical_type
...
def seeingLambda(w, FWHM, pivot=500.0 * u.nm):
...
def makeSource(args):
    ...
    if args.model[0].lower() == "constant":
        ...
        sourceSpectrum = SourceSpectrum(
            ConstFlux1D, amplitude=args.mag * u.ABmag
        )  # start with AB mag
        ...
        raise Exception("Invalid source model")
    ...
    if needNorm:
        ...
        if args.magfilter.upper() in [
            "USER",
            "MATCH",
        ]:  # MATCH is preferred, keeping USER as legacy
            # Use the wavelength range from command line
            from synphot.models import Box1D

            norm_band = SpectralElement(
                Box1D,
                amplitude=1,
                x_0=args.wrange.mean(),
        ...
        if args.magsystem.upper() == "AB":
            sourceSpectrum = sourceSpectrum.normalize(
                args.mag * u.ABmag, band=norm_band
            )
            # ,wavelengths=sourceSpectrum.waveset)
        elif args.magsystem.upper() == "VEGA":
            sourceSpectrum = sourceSpectrum.normalize(
                args.mag * uu.VEGAMAG, band=norm_band, vegaspec=vegaspec
            )
...
def make_empirical(spec, lams):
    ...
    return SourceSpectrum(
        Empirical1D, points=lams, lookup_table=spec(lams), keep_neg=True
    )
...
def makeLSFkernel(
    slit_w, seeing, ch, kernel_upsample=10.0, kernel_range_factor=4.0, pivot=500 * u.nm
):
    ...
    sigma_seeing_lam = sigma_seeing.to("pix", equivalencies=plate_scale[ch]).to(
        u.AA, equivalencies=dispersion_scale_nobin[ch]
    )
    slit_w_lam = slit_w.to("pix", equivalencies=plate_scale[ch]).to(
        u.AA, equivalencies=dispersion_scale_nobin[ch]
    )
    ...
    dlambda = (
        max(min(sigma_seeing_lam, slit_w_lam / 2.0), LSFsigma[ch]) / kernel_upsample
    )

    # Make wavelength array for sampling kernel
    ## KERNEL RANGE MUST HAVE 0 IN THE EXACT CENTER (need odd array length)
    xk = rangeQ(0.0 * dlambda, kernel_range + dlambda, dlambda)
    ...
    slitLSF = Gaussian1D(stddev=sigma_seeing_lam) * Box1D(
        width=slit_w_lam
    )  # Multiply seeing profile with slit "tophat"
...
def makeLSFkernel_slicer(
    slit_w,
    seeing,
    ch,
    kernel_upsample=10.0,
    kernel_range_factor=4.0,
    pivot=500 * u.nm,
    centeronly=False,
):
    """Placeholder until we have LSF data"""
    ...
    LSF = {}
    w_offsets = {"center": 0.0}
    if not centeronly:
        w_offsets["side"] = slit_w_lam
    ...
    for s, w_offset in w_offsets.items():  # slit offset=0, slice offset = slid width
        ...
        slitLSF = Gaussian1D(mean=-w_offset, stddev=sigma_seeing_lam) * Box1D(
            width=slit_w_lam
        )  # Multiply seeing profile with slit "tophat"
        instLSF = Gaussian1D(stddev=LSFsigma[ch])
        kernel1 = slitLSF(xk).value
        ...
        LSF[s] = {"kernel": _kernel, "fwhm": _fwhm[0], "dlambda": dlambda, "xk": xk}

    # fwhm = [x.value for x in fwhm]*fwhm[0].unit

    if not centeronly:
        LSF["side2"] = LSF["side"]
        LSF["side2"]["kernel"] = LSF["side2"]["kernel"][
            ::-1
        ]  ### Reverse direction for the opposite slice
...
def convolveLSF_old(
    ...
    kernel, fwhm, dlambda = makeLSFkernel(
        slit_w, seeing, ch, kernel_upsample, kernel_range_factor, pivot
    )

    # Make wavelength array for sampling spectrum; same spacing, larger range
    if wrange_ is None:
        wrange_ = channelRange[ch]
    x = rangeQ(
        wrange_[0] - kernel_range_factor * fwhm,
        wrange_[1] + kernel_range_factor * fwhm,
        dlambda,
    )

    # Convolved spectrum as unitless array
    spec2 = (
        convolve(spectrum(x).value, kernel, mode="same", method="auto") / kernel.sum()
    )
    ...
    if isinstance(spectrum, SourceSpectrum):
        newspec = SourceSpectrum(
            Empirical1D, points=x, lookup_table=spec2 * spectrum(1).unit, keep_neg=True
        )
    elif isinstance(spectrum, SpectralElement):
        newspec = SpectralElement(
            Empirical1D, points=x, lookup_table=spec2, keep_neg=True
        )
    else:
...
def convolveLSF(LSF, spectrum, ch, kernel_range_factor=4.0, wrange_=None):
    """Convolve spectrum at focal plane with LSF"""
    """
    Approximates seeing for each channel as Gaussian with scale at channel center wavelength
    Final kernel is INSTRUMENT * (SLIT X SEEING)  (*=convolve)
...
def sharpess_scalefree(theta, x=0.0, beta=moffat_beta):
...
def slitFractions(lam, w, h, FWHM, pivot=500.0 * u.nm):
    """Compute fraction of PSF passing through slit and side slices. Assumes Moffat PSF"""

    ts = moffat_theta_factor * seeingLambda(
        lam, FWHM, pivot=pivot
    )  # specific to Moffat PSF
    ...
    centerFrac = evaluate2Dinterp(PSFsum2D, wts / 2.0, hts / 2.0)
    totalFrac = evaluate2Dinterp(PSFsum2D, 3.0 * wts / 2.0, hts / 2.0)

    sideFrac = (totalFrac - centerFrac) / 2.0
    return {"total": totalFrac, "center": centerFrac, "side": sideFrac}


def slitEfficiency(w, h, FWHM, pivot=500.0 * u.nm, optics=None):
    ...
    ts = moffat_theta_factor * seeingLambda(
        lams, FWHM, pivot=pivot
    )  # specific to Moffat PSF
    # if ts.isscalar: ts=[ts]

    centerFrac = evaluate2Dinterp(PSFsum2D, w / ts / 2.0, h / ts / 2.0)
    totalFrac = evaluate2Dinterp(PSFsum2D, 3.0 * w / ts / 2.0, h / ts / 2.0)
    ...
    throughput_slicer = {}
    throughput_slicer["center"] = SpectralElement(
        Empirical1D, points=lams, lookup_table=centerFrac
    )
    throughput_slicer["side"] = SpectralElement(
        Empirical1D, points=lams, lookup_table=sideFrac
    )
    throughput_slicer["total"] = SpectralElement(
        Empirical1D, points=lams, lookup_table=totalFrac
    )

    return throughput_slicer


def profileOnDetector(
    channel, slit_w, seeing, pivot, lams, spatial_range=None, bin_spatial=1
):
...
def applySlit(
    ...
    if POINTSOURCE:
        ...
        throughput_slicer = slitEfficiency(
            slitw,
            slit_h,
            args.seeing[0],
            pivot=args.seeing[1],
        ...
        profile_slit = {
            k: profileOnDetector(
                ...
                binCenters[
                    k
                ].mean(),  ### If not using mean, must change slitFractions() to allow lambda arrays
                spatial_range=None,
                bin_spatial=args.binspat,
            )
            for k in chanlist
        }
        Npix_spatial = None
    ...
    if args.fastSNR:
        Npix_spatial = 2 * args.fastSNR  # overrides extended source size

    sharpness = {
        k: {
            s: 1.0 / array([Npix_spatial] * len(binCenters[k]))
            if Npix_spatial is not None  # 1/sharpness = [N, N, N...]
    ...
    for k in chanlist:
        ...
        if args.hires:
            ...
            LSF = makeLSFkernel_slicer(
                ...
                args.seeing[0],
                k,
                kernel_range_factor=kernel_range_factor,
                pivot=args.seeing[1],
            )
        ...
        for s in slicer_paths:
            if POINTSOURCE:
                spec = (
                    source_at_slit[k] * throughput_slicer[s]
                )  # This applies slit loss and optics
                ...
                spec = (
                    source_at_slit[k] * bg_pix_area * Npix_spatial * args.binspat
                )  # signal per pixel * N_pixels
                if s == "side":
                    spec *= throughput_slicerOptics
...
def applySlit_extended(
    ...
    if args.fastSNR:
        Npix_spatial = 2 * args.fastSNR
    elif args.extended != None:
        Npix_spatial = args.extended
    else:
        Npix_spatial = None
    ...
    sharpness = {
        k: {
            s: 1.0
            / array([Npix_spatial] * len(binCenters[k]))  # 1/sharpness = [N, N, N...]
            for s in slicer_paths
        }
        for k in chanlist
    }
    ...
    for k in chanlist:
        ...
        for s in slicer_paths:
            spec = (
                source_at_slit[k] * bg_pix_area * Npix_spatial
            )  # signal per pixel * N_pixels
            if s == "side":
                spec *= throughput_slicerOptics
            if args.hires:
...
def computeSNR(exptime, slitw, args, SSSfocalplane, allChans=False):
    ...
    for k in chanlist:
        ...
        for s in slicer_paths:
            ...
            NOISE2 = (
                bgvar[k][s] * exptime + darkcurrent[k] * exptime * u.pix
            ) * args.binspat
            NOISE2 += (
                readnoise[k] * u.pix
            ) ** 2 / u.ct  # read noise per read pixel is unchanged
            NOISE2 = SIGNAL + NOISE2 / sharpness[k][s]  # add shot noise

            SNR2[k][s] = (SIGNAL**2 / NOISE2).to("ct").value

        if args.noslicer or args.fastSNR:
            SNR[k] = SNR2[k]["center"] ** 0.5
        else:
            SNR[k] = (
                SNR2[k]["center"] + 2 * SNR2[k]["side"]
            ) ** 0.5  # SNR combines in quadrature
    ...
    # Return 1 number - the SNR caclulated from user input
        ...
        return SNR[args.channel].mean()


def plotAllChannels(
    spec, lambda_range=None, binned=False, spec_allchan=None, binCenters=None
):
    """
    ...
    if lambda_range is not None:
        ax.axvspan(
            lambda_range[0], lambda_range[1], alpha=0.2, color="grey"
        )  # shade user range
