...
def plotSNR_vs_slit(args, plt):
    ...
    w_arcsec = arange(0.2, 4.5, 0.2)

    ### HARDCODED UNITS
    fwhm_dlam = [
        makeLSFkernel_slicer(
            wi, args.seeing[0], args.channel, pivot=args.seeing[1], kernel_upsample=10.0
        )[1:]
        for wi in w_arcsec * u.arcsec
    ]
    ...
    for s, c, l in zip(slicer, colors, labels):
        ...
        args.noslicer = not s
        result, efffunc, SNRfunc = main(
            args, quiet=True, ETCextras=True
        )  # SNRfunc will be SNR vs slit in arcsec
    ...
    ax1.set_ylabel("SNR")
    ax1.set_title(
        '%s   mag=%s   exptime=%s   seeing=%s"'
        % (
            result["wrange"],
            args.mag,
            result["exptime"].round(3),
            args.seeing[0].round(2).value,
        )
    )
    ...
    ax2.tick_params(
        axis="y", direction="in", length=6, width=2, colors="g"
...
def plotSNR_vs_slit_mags(args, plt):
    ...
    secax.tick_params(
        axis="x", direction="in", length=6, width=2, colors="black"
    )  # , grid_color='r', grid_alpha=0.5)

    fig.suptitle(
        '%s   skymag=%s   exptime=%s   seeing=%s"'
        % (
            result["wrange"],
            args.skymag,
            result["exptime"].round(3),
