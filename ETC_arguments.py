...
help = (
    "Increase accuracy when solving for slit width and exptime simultaneously. (slower)"
)
...
sourceparam_req.add_argument(
    "-magsystem", type=str, choices=choices, required=True, help=help
)

help = """Johnson filter (UBVRIJK) to define source magnitude. Use FILTER="match" to normalize to the WRANGE input"""
choices = (
    [c for c in "UBVRIJK"] + ["user", "USER", "User"] + ["match", "MATCH", "Match"]
)
sourceparam_req.add_argument(
    "-magfilter", type=str, choices=choices, required=True, help=help
)
...
help = (
    'Astronomical source model.  Examples: "constant" (default), "blackbody 5000", "template spiral_001".  \
The "constant" model ignores other parameters in this group.'
)
sourceparam_add.add_argument(
    "-model", nargs="+", required=False, default=["constant"], help=help
)

help = "Redshift"
sourceparam_add.add_argument("-z", type=nonegfloat, default=0.0, help=help)

help = "Selective Extinction E(B-V); default=0"
sourceparam_add.add_argument("-E_BV", type=float, default=0.0, help=help)
...
etc_args = [
    ...
    "wrange",
    "exptime",
]  # Order is important  #, 'SNR' now coded in exptime
etc_kwargs = [
    "slitwidth",
    "airmass",
    "skymag",
...
etc_optkwargs = [
    "binspect",
    "binspat",
]  # , 'fast_SNR', -noslicer ; these take no arguments  ### -binspat ,-binspect


def formETCcommand(
    row,
):  ### Maybe make command as list not a big string  ### standardize 1 col per kw; make OTM reformat
    ...
    cmd_optkwargs = [
        "-%s %s" % (k, row[k]) for k in cols_exist if not is_masked(row[k])
    ]
...
def check_inputs_add_units(args):
    ...
    if model.lower() not in choices:
        parser.error("-model first argument must be in " + str(choices.keys()))

    if len(args.model) != choices[model]:
        parser.error(
            '-model "%s" requires exactly %i arguments' % (model, choices[model])
        )
    ...
    if model.lower() == "template":
        ...
        if not foundTemplate:
            parser.error("Could not find source template: " + args.srctemp + ".fits")
    ...
    if model.lower() == "blackbody":
        try:
            args.tempK = posfloat(
                args.model[1]
            )  # copy blackbody temperature to new attribute
        except:
            parser.error("-model blackbody TEMPK requires TEMPK to be a positive float")
    ...
    if args.extmodel not in choices:
        parser.error("-extmodel must be in " + str(choices))

    ## Check for Valid slit mode ##
    # Check mode string
    args.slitmode = args.slit[0].upper()  # move the code to new parameter
    if args.slitmode not in slitmodes.keys():
        parser.error("-slitwidth MODE must be in " + str(list(slitmodes.keys())))
    # Convert AUTO to SNR
    ...
    # Check for extra parameter
        ...
        parser.error(
            "-slitwidth %s needs a parameter X in range %s"
            % (args.slitmode, str(slitmodes[args.slitmode]))
        )
    # Check parameter range
    if (args.slit > max(slitmodes[args.slitmode])) or (
        args.slit < min(slitmodes[args.slitmode])
    ):
        parser.error(
            "-slitwidth %s X requires X be in range %s"
            % (args.slitmode, str(slitmodes[args.slitmode]))
        )
    ...
    if args.slitmode == "SET":
        args.slit = (
            slitfloat(args.slit) * u.arcsec
        )  ### checks for valid range -- should simplify
    ...
    if args.wrange[0] < channelRange[args.channel][0]:
        parser.error(
            "Wavelength range %s not in channel %s" % (str(args.wrange), args.channel)
        )
